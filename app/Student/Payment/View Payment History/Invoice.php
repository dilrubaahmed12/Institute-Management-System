...
<?php
    ...
    $sql = "SELECT payment_id FROM payment WHERE payment_for_month='$pay_month' AND payment_year='$pay_year' AND student_id='$std_id' ";
    $result = $conn->query($sql);

    if($result->num_rows == 1){
        $invoice_cat = 'new_payment';
    }else{
        $sql = "SELECT MAX(payment_date) AS 'max_date' FROM payment WHERE payment_for_month='$pay_month' AND payment_year='$pay_year' AND student_id='$std_id' ";
        $data = $conn->query($sql);
        $row = $data->fetch_assoc();
        
        if(DateTime::createFromFormat('d/m/Y', $pay_date)->format('Y-m-d') == $row['max_date']){
            $invoice_cat = 'clear_due';
            $pre_due = $paid_amount;
        }else{
            $invoice_cat = 'new_payment';
            $due = (show_rcvbable_amount($std_id) - (float)$discount) - $paid_amount;
        }
    }       
?>
                    ...
                    <div style="float: left; width: 39%; ">
                        <div style="text-align: left;  padding-left: 5.7%; padding-right: 2%; font-size: 12px; padding-top: 1px;">
                            <table style="font-size: 12px; line-height: 15px; font-family: Verdana, Geneva, sans-serif; width:100%;">
                                ...
                                ?>
                                <tr <?php if($invoice_cat != 'clear_due') echo 'hidden'; ?>>
                                    <td style="width: 58%; vertical-align: top;">
                                        Previous Due
                                    </td>
                                    <td style="width: 2%; vertical-align: top;">
                                ...
                                <tr <?php if($invoice_cat != 'new_payment') echo 'hidden'; ?>>
                                    <td style="width: 58%; vertical-align: top;">
                                        Total Payable
                                    </td>
                                    <td style="width: 2%; vertical-align: top;">
                                    ...
                                    <td style="width: 40%; vertical-align: top;">
                                        <span></span>
                                        <span><?php echo show_rcvbable_amount($std_id); ?></span>
                                    </td>
                                </tr>
                                <tr <?php if($invoice_cat != 'new_payment') echo 'hidden'; ?>>
                                    <td style="width: 58%; vertical-align: top;">
